...
export function FloatingAction() {
  ...
  // 🚀 Logic to move button up if "Custom Combo" bar is active (Mobile Only)
  // Usually, custom-combo has a sticky bar at the bottom.
  const isCustomCombo = pathname === "/custom-combo";

  return (
    <button
      onClick={handleClick}
      className={`fixed z-[90] w-14 h-14 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.3)] flex items-center justify-center transition-all duration-700 ease-in-out hover:scale-110 overflow-hidden ${
        mode === 'whatsapp' 
          ? 'bg-[#D4AF37] text-[#1E1B18] border-2 border-[#D4AF37]' 
          : 'bg-[#1E1B18] text-[#D4AF37] border-2 border-[#D4AF37]/50'
      } ${
        /* 📱 Mobile: Move button up if on custom-combo page to avoid overlap */
        isCustomCombo 
