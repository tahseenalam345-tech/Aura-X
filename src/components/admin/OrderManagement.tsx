...
export default function OrderManagement() {
  ...
  return (
    <div className="min-h-screen bg-[#121212] p-2 md:p-6 font-sans text-gray-200 pb-24 w-full">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5 bg-[#1E1E1E] p-1.5 rounded-xl border border-gray-800 w-full xl:w-auto">
            ...
            <div className="flex items-center gap-1.5 px-2 border-l border-gray-700 w-full md:w-auto mt-2 md:mt-0 justify-center">
                ...
                {dateFilter === "custom" && (
                    <button onClick={() => { setDateFilter("this_month"); setCustomDate(getCurrentMonthDates()); }} className="ml-1 text-red-500 hover:text-red-400"><X size={14}/></button>
                )}
            </div>
        </div>
      </div>
      ...
      {/* 🚀 THE MASTER TABLE (SCROLL-LOCKED & MOBILE RESPONSIVE) */}
      <div className="bg-[#1E1E1E] border border-gray-800 rounded-xl overflow-hidden mb-8 shadow-lg">
        {/* max-h-[70vh] locks the height so scrollbar is always visible at bottom of screen */}
        <div className="overflow-auto custom-scrollbar w-full max-h-[70vh]">
          {/* Use 'block md:table' to convert table into cards on mobile */}
          <table className="w-full text-left border-collapse min-w-full md:min-w-[2000px] block md:table">
            ...
            <thead className="hidden md:table-header-group">
              <tr>
                ...
                <th className="p-3 text-[10px] font-bold text-gray-300 uppercase tracking-widest bg-red-900/30 border-r border-red-900/50 w-32 sticky top-0 z-20 shadow-sm">Actual Cost</th>
                <th className="p-3 text-[10px] font-bold text-gray-300 uppercase tracking-widest bg-yellow-900/30 border-r border-yellow-900/50 w-40 sticky top-0 z-20 shadow-sm">DC + Extras</th>
                <th className="p-3 text-[10px] font-bold text-gray-300 uppercase tracking-widest bg-green-900/30 border-r border-green-900/50 w-32 sticky top-0 z-20 shadow-sm">Sold Price</th>
                <th className="p-3 text-[10px] font-bold text-gray-300 uppercase tracking-widest bg-[#333] border-r border-gray-700 w-32 sticky top-0 z-20 shadow-sm">4% Tax Calc</th>
                <th className="p-3 text-[10px] font-bold text-aura-gold uppercase tracking-widest bg-aura-gold/10 border-r border-aura-gold/20 w-36 sticky top-0 z-20 shadow-sm">Net Profit</th>
                ...
                <th className="p-3 text-[10px] font-bold text-red-400 uppercase tracking-widest w-16 text-center sticky top-0 bg-[#252525] z-20 shadow-sm">DEL</th>
            ...
            {/* Table Body - Becomes Stacked Cards on Mobile */}
            <tbody className="block md:table-row-group divide-y-0 md:divide-y divide-gray-800 p-2 md:p-0">
              {filteredOrders.map(order => {
                const { taxAmount, afterTax, profit } = calculateOrderFinances(order);
                const isDelivered = order.status === "Delivered";

                return (
                  <tr key={order.id} className="flex flex-col md:table-row mb-6 md:mb-0 bg-[#1A1A1A] md:bg-transparent border border-gray-700 md:border-none rounded-xl md:rounded-none overflow-hidden shadow-md md:shadow-none hover:bg-gray-800/50 transition-colors group">
                    
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex md:table-cell justify-between items-center md:items-start gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Order Date</span>
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 md:bg-gray-800/30 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Status Date</span>
                      <div className="w-2/3 md:w-full">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Order ID</span>
                      <div className="w-2/3 md:w-full">
                        <input type="text" value={order.order_id || ""} onChange={(e) => handleEditLocal(order.id, 'order_id', e.target.value)} onBlur={(e) => handleBlurOrChange(order.id, 'order_id', e.target.value)} className="bg-transparent text-xs font-bold text-gray-200 outline-none w-full border-b border-transparent focus:border-aura-gold uppercase" />
                      </div>
                    </td>
                    
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex flex-col md:table-cell gap-2">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest">Customer Info</span>
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex flex-col md:table-cell gap-2">
                       <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest">Product Details</span>
                       <div className="w-full">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Courier</span>
                      <div className="w-2/3 md:w-full">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Status</span>
                      <div className="w-2/3 md:w-full">
                        <select value={order.status} onChange={(e) => { handleEditLocal(order.id, 'status', e.target.value); handleBlurOrChange(order.id, 'status', e.target.value); }} className={`text-[10px] font-bold px-2 w-full py-2 md:py-1.5 rounded-lg border outline-none cursor-pointer text-center shadow-sm ${getStatusColor(order.status)}`}>
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Payment</span>
                      <div className="w-2/3 md:w-full">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-red-900/20 md:bg-red-900/10 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Actual Cost</span>
                      <div className="w-2/3 md:w-full flex items-center gap-1 text-xs font-bold text-gray-300">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-yellow-900/20 md:bg-yellow-900/10 flex flex-col md:table-cell gap-2">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest">DC & Extras</span>
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-green-900/20 md:bg-green-900/10 flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">Sold Price</span>
                      <div className="w-2/3 md:w-full flex items-center gap-1 text-sm font-black text-green-400">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-700 md:bg-gray-800/50 flex flex-col md:table-cell gap-2">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest">Tax Logic</span>
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-aura-gold/10 md:bg-aura-gold/5 relative flex md:table-cell justify-between items-center gap-4">
                      <span className="md:hidden text-[10px] text-aura-gold font-bold uppercase tracking-widest w-1/3">Net Profit</span>
                      <div className="w-2/3 md:w-full relative">
                        {order.status === "Return" && (
                          <div className="absolute inset-0 bg-red-900/80 flex items-center justify-center backdrop-blur-[1px] z-10 rounded">
                    ...
                    <td className="p-3 border-b md:border-b-0 md:border-r border-gray-800 flex flex-col md:table-cell gap-2">
                      <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest">Notes</span>
                      <div className="w-full">
                        <textarea 
                          ...
                          className="bg-transparent border border-dashed border-gray-700 focus:border-solid focus:border-aura-gold focus:bg-[#252525] rounded p-1.5 text-[10px] outline-none w-full h-12 md:h-12 resize-none transition-all text-gray-300"
                        />
                      </div>
                    </td>
                    
                    <td className="p-3 border-b md:border-b-0 border-gray-800 text-center flex md:table-cell justify-center items-center">
                        <button onClick={() => handleDelete(order.id)} className="w-full md:w-auto p-2 md:p-1.5 bg-red-900/20 md:bg-transparent text-red-500 md:text-gray-500 hover:bg-red-900/50 hover:text-red-400 rounded-lg transition-colors flex items-center justify-center gap-2">
                            <Trash2 size={16}/>
                            <span className="md:hidden text-xs font-bold uppercase tracking-widest">Delete Order</span>
                        </button>
                    </td>

                    {customColumns.map(col => (
                      <td key={col} className="p-3 border-r border-gray-800 flex md:table-cell justify-between items-center gap-4">
                        <span className="md:hidden text-[10px] text-gray-500 font-bold uppercase tracking-widest w-1/3">{col}</span>
                        <div className="w-2/3 md:w-full">
