...
const COLOR_MAP: Record<string, string> = {
  ...
  "Rose Gold": "#B76E79",
  "Black": "#000000",
  "Two-Tone (Silver/Gold)": "#F5F5DC",
  "Two-Tone (Silver/Rose)": "#FFE4E1",
  "Blue": "#0000FF",
  "Green": "#008000",
  "White": "#FFFFFF",
  "Brown": "#A52A2A",
...
export default function InventoryTab({ products, fetchProducts }: { products: any[], fetchProducts: () => void }) {
  ...
  const initialFormState = {
    ...
    waterResistance: "0ATM (No Resistance)", 
    glass: "", 
    caseMaterial: "", 
    caseColor: "Silver", caseShape: "Round", 
    caseDiameter: "40mm", 
    caseThickness: "10mm", 
    strapMaterial: "", 
    strapColor: "", 
    strapWidth: "20mm", adjustable: true,
    dialColor: "", 
    luminous: false, 
    dateDisplay: false, 
    weight: "135g", 
    ...
    shippingText: "2-4 Working Days", returnPolicy: "7 Days Return Policy", 
    boxIncluded: false, 
  ...
  const handlePublish = async (e: React.FormEvent) => {
      ...
      const productPayload = {
          ...
          description: formData.description, main_image: formData.mainImage, 
          tags: tagsArray, 
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {["Featured", "Sale", "Limited Edition", "Fire", "New Arrival", "Best Seller"].map(tag => (
                                                <button 
                                                    ...
                                                    className={`px-3 py-1 rounded-full text-xs font-bold border transition-all ${formData.tags === tag ? 'bg-aura-brown text-white border-aura-brown' : 'bg-white text-gray-400 border-gray-200'}`}
                                                >
                                                    {tag}
                                                </button>
                                            ))}
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="flex flex-col md:flex-row gap-6 mb-6">
                                    <div className="w-full md:w-40">
                                        ...
                                        <div 
                                            className={`w-full h-40 rounded-2xl border-2 border-dashed flex items-center justify-center relative overflow-hidden cursor-pointer hover:border-aura-gold bg-white ${formData.mainImage ? 'border-aura-gold' : 'border-gray-300'}`}
                                            onDragOver={(e) => e.preventDefault()}
                                            onDrop={(e) => handleDrop(e, 'main')}
                                            onPaste={(e) => handlePaste(e, 'main')}
                                            tabIndex={0}
                                        >
                                            {formData.mainImage ? (
                                                <>
                                                ...
                                                <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer">
                                                    ...
                                                    <span className="text-xs text-gray-400 mt-1">Click / Paste / Drop</span>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex flex-wrap gap-4">
                                            ...
                                            <div 
                                                className="w-24 h-24 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center cursor-pointer hover:border-aura-gold flex-shrink-0 bg-white"
                                                onDragOver={(e) => e.preventDefault()}
                                    ...
                                    <div className="w-full md:w-40">
                                        ...
                                        <div 
                                            className={`w-full h-40 rounded-2xl border-2 border-dashed flex items-center justify-center relative overflow-hidden cursor-pointer hover:border-aura-gold bg-white ${formData.video ? 'border-aura-gold' : 'border-gray-300'}`}
                                            onDragOver={(e) => e.preventDefault()}
                                        ...
                                        >
                                            {formData.video ? (
                                                ...
                                                <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer">
                                                    ...
                                                    <span className="text-xs text-gray-400 mt-1">Add Video</span>
                                            ...
                                            <div key={index} className="flex flex-col md:flex-row gap-4 items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                                                ...
                                                <select 
                                                    ...
                                                    onChange={(e) => { 
                                                        ...
                                                        c[index].name = name; 
                                                        // AUTO SELECT COLOR HEX based on name
                            ...
                            <section className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select className="w-full p-3 bg-white border rounded-xl" value={formData.caseMaterial || ""} onChange={e => setFormData({...formData, caseMaterial: e.target.value})}>
                                            <option value="">Select Material</option>
                                            <option>Stainless Steel</option><option>Alloy</option><option>Titanium</option>
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select className="w-full p-3 bg-white border rounded-xl" value={formData.glass || ""} onChange={e => setFormData({...formData, glass: e.target.value})}>
                                            <option value="">Select Glass</option>
                                            <option>Mineral</option><option>Sapphire</option><option>Hardlex</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-gray-500">Dial Color</label>
                                        <select className="w-full p-3 bg-white border rounded-xl" value={formData.dialColor || ""} onChange={e => setFormData({...formData, dialColor: e.target.value})}>
                                            <option value="">Select Color</option>
                                            {POPULAR_COLORS.map(c => <option key={c}>{c}</option>)}
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select className="w-full p-3 bg-white border rounded-xl" value={formData.strapMaterial || ""} onChange={e => setFormData({...formData, strapMaterial: e.target.value})}>
                                            <option value="">Select Material</option>
                                            <option>Leather</option><option>Metal</option><option>Chain</option><option>Silicon</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-gray-500">Strap Color</label>
                                        <select className="w-full p-3 bg-white border rounded-xl" value={formData.strapColor || ""} onChange={e => setFormData({...formData, strapColor: e.target.value})}>
                                            <option value="">Select Color</option>
                                            {POPULAR_COLORS.map(c=><option key={c}>{c}</option>)}
                                        </select>
                                    </div>
