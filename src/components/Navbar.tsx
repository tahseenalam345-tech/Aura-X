...
const categoryStructure = [
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: "Wireless Earbuds", href: "/category/earbuds" },
    ]
  },
  {
    name: "Fragrances",
    ...
    subItems: [
      { name: "Men's Perfume", href: "/category/perfume-men" },
      { name: "Women's Perfume", href: "/category/perfume-women" },
    ]
  },
  {
    name: "Combos",
...
export function Navbar() {
  ...
  return (
    <>
               ...
               <div key={cat.name} className="relative group h-full flex items-center">
                 <Link 
                   ...
                   className={`flex items-center gap-1 text-[11px] font-black tracking-[0.2em] uppercase transition-all duration-300 py-2 
                      ${cat.isSpecial ? 'text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#F9E596] animate-pulse drop-shadow-sm' : 'text-[#3A2A18] hover:text-[#D4AF37]'}`}
                 >
                    {cat.name}
                      ...
                      <div className="bg-white/90 backdrop-blur-2xl border border-white/60 shadow-[0_20px_40px_rgba(0,0,0,0.08)] rounded-2xl overflow-hidden flex flex-col py-2">
                        {cat.subItems.map((sub) => (
                          <Link 
                            key={sub.name} 
                            href={sub.href}
                            className="px-6 py-3 text-[10px] font-bold text-[#3A2A18] hover:text-[#D4AF37] hover:bg-[#FDFBF7] transition-colors uppercase tracking-[0.2em] flex items-center justify-between group/sub"
                          >
                            {sub.name}
      ...
      {/* 🚀 PREMIUM MOBILE SIDEBAR (WITH ACCORDION CATEGORIES) */}
      <AnimatePresence>
        {isSidebarOpen && (
          <>
                   ...
                   {categoryStructure.map((cat) => (
                     <div key={cat.name} className="flex flex-col border-b border-[#D4AF37]/10 pb-4">
                       <div 
                         ...
                         onClick={() => cat.subItems ? toggleCategory(cat.name) : handleCloseAll()}
                       >
                         <Link 
                           href={cat.href}
                           onClick={(e) => cat.subItems && e.preventDefault()} 
                           className={`text-2xl font-serif transition-all duration-300 ${cat.isSpecial ? 'text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#8B7355] font-black animate-pulse' : 'text-[#1E1B18] group-hover:text-[#D4AF37]'}`}
                         >
                            {cat.name}
                         </Link>
                         {cat.subItems && (
                           <button className="p-2 rounded-full hover:bg-white/50 transition-colors">
                             <ChevronDown className={`w-5 h-5 text-[#D4AF37] transition-transform duration-300 ${expandedCategory === cat.name ? '-rotate-180' : ''}`} />
                           </button>
                         )}
                       </div>
                       ...
                       <AnimatePresence>
                         {cat.subItems && expandedCategory === cat.name && (
                           <motion.div 
                             ...
                             className="overflow-hidden flex flex-col gap-4 pl-4 mt-4 border-l-2 border-[#D4AF37]/30 ml-2"
                           >
                             {cat.subItems.map(sub => (
                               <Link 
                                 key={sub.name} 
                                 ...
                                 className="text-xs font-bold uppercase tracking-[0.2em] text-[#8B7355] hover:text-[#D4AF37] flex items-center gap-2"
                               >
                                 <span className="w-1 h-1 rounded-full bg-[#D4AF37]"></span> {sub.name}
                               </Link>
                             ))}
                           </motion.div>
                         )}
