...
import { ChevronDown, Filter, X, Loader2, Zap, LayoutGrid, User, Sparkles, Users, Truck, ShieldCheck, Flame, Check } from "lucide-react"; 
...
export default function CategoryPage() {
  ...
  const [visibleCount, setVisibleCount] = useState(isReturning ? globalVisibleCount : 10);
  ...
  // 🚀 AUTO-LOAD TIMER LOGIC (Har 2.5 sec baad 10 aur load honge)
  useEffect(() => {
      if (!loading && visibleCount < products.length) {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    // 🚀 NEW MIXED PRIORITY LOGIC (If "featured" is selected)
    if (sortBy === "featured") {
        const aPriority = a.priority || 0;
        const bPriority = b.priority || 0;
        if (aPriority !== bPriority) return bPriority - aPriority; // Highest priority first
        // If priority is same, fallback to newest
  ...
  return (
    <main className="min-h-screen bg-[#FDFBF7] text-aura-brown font-serif">
      ...
      {/* 🚀 1. FULL WIDTH PROMO BANNER WITH URDU TEXT */}
      <div className="w-full bg-gradient-to-r from-[#D4AF37] via-[#F9E596] to-[#D4AF37] py-3 md:py-3.5 rounded-xl mb-6 z-40 relative shadow-[0_5px_15px_rgba(212,175,55,0.3)] border border-[#8B7355]/30 flex justify-center items-center gap-2 mt-[56px] md:mt-[80px]">
